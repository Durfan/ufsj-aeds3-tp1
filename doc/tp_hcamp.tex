...
\begin{algorithm}[h]
	\small
	\caption{Ache o conjunto máximo de triângulos}
	\label{Algorithm:findMAX}
	\begin{algorithmic}[1]
		\Procedure{$\mathbf{findMAX}$}{Conjunto $P$}
		\While {$n$}
			\While {$n$}
				\If {$n-1$ está dentro do triângulo testado}
				\State \textbf{insira em outro conjunto}
				\EndIf
			\EndWhile
		\EndWhile
		\EndProcedure
